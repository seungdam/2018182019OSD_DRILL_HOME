...
class Boy:
    ...
    def update(self):
        ...
        if self.state is JUMPING:
            ...
            self.jump_time -= 1
            if (self.collide_brick and self.y < 200) or self.jump_time is 0:
                self.jump_time = 100
                self.state = FALLING
        elif self.state is FALLING:
            self.y -= self.jump_power * game_framework.frame_time

            if self.collide_brick:
                if self.y > 220:
                    self.state = IDLE
            elif self.collide_grass:
                self.state = IDLE
        elif self.state is IDLE:
            if self.collide_brick or self.collide_grass:
                if self.collide_brick:
                    self.move = True
                pass
            elif self.collide_brick is False and self.collide_grass is False:
                self.y -= self.jump_power * game_framework.frame_time
                self.move = False
